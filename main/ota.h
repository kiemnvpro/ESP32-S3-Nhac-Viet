...
public:
    ...
    std::string GetCheckVersionUrl();
	    // --- Assets manifest helpers (để Application::CheckAssetsVersion dùng) ---
    void SetAssetsManifest(const std::string& url, const std::string& version, const std::string& sha256);
    const std::string& AssetsUrl() const { return assets_url_; }
    const std::string& AssetsVersion() const { return assets_version_; }
    const std::string& AssetsSha256() const { return assets_sha256_; }
    void ClearAssetsManifest();
