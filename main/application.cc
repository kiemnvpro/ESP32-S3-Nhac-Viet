...
bool success = assets.Download(
    ...
    [this, display,
     last_progress = -1,
     last_tick = (uint32_t)0] (int progress, size_t speed) mutable
    {
        // Giảm tần suất cập nhật để đỡ load LVGL
        uint32_t now = xTaskGetTickCount();
        if (progress == last_progress || (now - last_tick) < pdMS_TO_TICKS(300)) return;
        last_progress = progress;
        last_tick = now;

        char buf[48];
        // Ghi vào biến thành viên để đảm bảo lifetime
        snprintf(buf, sizeof(buf), "%d%% %uKB/s", progress, (unsigned)(speed / 1024));
        this->m_assets_progress_text.assign(buf);

        // Đẩy cập nhật UI chạy trên thread an toàn của app
        this->Schedule([this, display]() {
            // TRUYỀN con trỏ tới chuỗi THÀNH VIÊN (sống lâu), không phải biến cục bộ
            display->SetChatMessage("system", this->m_assets_progress_text.c_str());
        });
    }
);
    ...
    // Apply assets
    assets.Apply();
	// Dọn cờ liên quan đến OTA assets để tránh tải lại sau reboot
{
    Settings s("assets", true);
    s.EraseKey("download_url");
    s.EraseKey("version");
    s.EraseKey("sha256");
    
}

// Xoá tin nhắn tiến độ và đặt emotion lại bình thường
...
void Application::Start() {
    ...
    if (protocol_started) {
        std::string message = std::string(Lang::Strings::KIEMNV_VERSION) + ota.GetCurrentVersion();
...
void Application::MainEventLoop() {
    while (true) {
		...
		// Gọi hình nền ra
		#ifdef HAVE_LVGL
            WallpaperManager::GetInstance().OnTick();
...
void Application::SetDeviceState(DeviceState state) {
    ...
    led->OnStateChanged();
    // Khi trạng thái chờ chuyển sang bất kỳ trạng thái nào khác, dừng phát nhạc
