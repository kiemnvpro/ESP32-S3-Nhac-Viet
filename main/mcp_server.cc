...
// Danh sách file hình nền 2/3
// ---- Wallpaper control ----
{
    AddTool("self.wallpaper.apply",
        "Đổi hình nền theo số (bắt đầu từ 1). "
        ...
        [](const PropertyList& properties) -> ReturnValue {
            int one_based = properties["index"].value<int>();
            auto effect_str = properties["effect"].value<std::string>();
            std::transform(effect_str.begin(), effect_str.end(), effect_str.begin(), ::tolower);

            WallpaperManager::TransitionEffect fx = WallpaperManager::TransitionEffect::FadeBlack;
            if (effect_str == "fade_black") fx = WallpaperManager::TransitionEffect::FadeBlack;
            // Các tên khác cũng map về fade_black để không treo
            else fx = WallpaperManager::TransitionEffect::FadeBlack;

            size_t idx0 = (one_based > 0) ? (size_t)(one_based - 1) : 0;
            bool ok = WallpaperManager::GetInstance().ApplyWithEffect(idx0, fx);
            return ok;
        });
}
// Danh sách file hình nền 3/3
// ---- Wallpaper timer control ----
{
    AddTool("self.wallpaper.set_interval",
        "Đặt thời gian tự động thay hình nền. "
        "Các giá trị hợp lệ: 30s, 1m, 3m, 5m, 15m, 30m, 1h.",
        PropertyList({
            Property("duration", kPropertyTypeString, "3m")
        }),
        [](const PropertyList& props) -> ReturnValue {
            auto dur = props["duration"].value<std::string>();
            std::transform(dur.begin(), dur.end(), dur.begin(), ::tolower);

            int seconds = 180; // mặc định 3 phút
            if (dur == "30s")  seconds = 30;
            else if (dur == "1m")  seconds = 60;
			else if (dur == "3m")  seconds = 3 * 60;
            else if (dur == "5m")  seconds = 5 * 60;
            else if (dur == "15m") seconds = 15 * 60;
            else if (dur == "30m") seconds = 30 * 60;
            else if (dur == "1h" || dur == "60m") seconds = 3600;

            WallpaperManager::GetInstance().EnableAutoRotate(true, seconds);
            ESP_LOGI("MCP", "Wallpaper auto-rotate interval set to %d seconds", seconds);
            return true;
        });
}
// Báo thức
...
AddTool("self.alarm.list",
    ...
    [&alarm](const PropertyList&) -> ReturnValue {
        std::string json; alarm.ListAlarms(json); return json;
    });

AddTool("self.alarm.clear",
    "Xoá toàn bộ báo thức đã đặt.",
    PropertyList(),
    [&alarm](const PropertyList&) -> ReturnValue {
        alarm.RemoveAll();
    ...
    if (camera) {
     ...
     if (music) {
         AddTool("self.music.play_song",
             "Phát bài hát đã chỉ định. Khi người dùng yêu cầu phát nhạc, công cụ này sẽ tự động lấy thông tin chi tiết về bài hát và bắt đầu phát trực tuyến.\n"
             "Tham số:\n"
             "  `song_name`: Tên bài hát (bắt buộc)\n"
             "  `artist_name`: Tên ca sĩ (tùy chọn, mặc định là chuỗi trống)\n"
             "Phát lại:\n"
             "  Thông tin trạng thái phát lại, phát bài hát ngay lập tức mà không cần xác nhận.",
             PropertyList({
                 Property("song_name", kPropertyTypeString),//Tên bài hát (bắt buộc)
                 Property("artist_name", kPropertyTypeString, "")//Tên ca sĩ (tùy chọn, mặc định là chuỗi trống)
             }),
             [music](const PropertyList& properties) -> ReturnValue {
                 ...
                 if (!music->Download(song_name, artist_name)) {
                     return "{\"success\": false, \"message\": \"Không thể lấy được bài nhạc\"}";
                 }
         ...
         AddTool("self.music.set_display_mode",
             "Thiết lập chế độ hiển thị cho việc phát nhạc. Bạn có thể chọn hiển thị phổ hoặc lời bài hát. Ví dụ: nếu người dùng nói 'Mở phổ' hoặc 'Hiển thị phổ', 'Mở lời bài hát' hoặc 'Hiển thị lời bài hát', chế độ hiển thị tương ứng sẽ được thiết lập.\n"
             "Tham số:\n"
             "  `mode`: Chế độ hiển thị, các giá trị tùy chọn là 'spectrum' hoặc 'lyrics'.\n"
             "Phát lại:\n"
             "  Đặt thông tin kết quả.",
             PropertyList({
